...
// peak | current, with each temp run colored by how it falls on the widget's
// green->red gradient. "current" is the live value so you can compare the
// real temp against the spec columns at a glance.
Row {
  id: spec
  ...
  // idle/load/peak use the same interpolated gradient as the bar temp text,
  // computed over the same [cool, hot] envelope.
  ...
  readonly property real colTotal: spec.colLabel + spec.colLoad + spec.colFreq + spec.colPeak + spec.colCur
